...
def adpcm_blocks_from_file(
    ...
    *,
    ...
    stats = _PcmStats()
    try:
        yield from adpcm_blocks_from_pcm_bytes(
            observe_pcm_blocks(iter_pcm_blocks(process.stdout, config.samples_per_block), stats), encoder
        )
    finally:
...
def adpcm_blocks_from_microphone(
    config: TalkConfig, *, volume: float = 1.0, seconds: float | None = None
) -> Iterator[bytes]:
    try:
